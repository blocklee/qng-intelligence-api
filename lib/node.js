...
Node.prototype.rpcRequest = async function (method, params) {
	let options = {
		...
		body: JSON.stringify({ "jsonrpc": "1.0", "id": "nodejs", "method": method, "params": params })
	};
...
Node.prototype.setupSockets = function () {
	...
	socket.on('open', async function open() {
		...
		console.info('   ', 'Trying to login', self.info);
		socket.emit('hello', {
			id: self.id,
			info: self.info,
			secret: WS_SECRET
...
Node.prototype.getStats = async function (forced) {
	...
	if (this._socket)
		this._lastStats = JSON.stringify(this.stats);
	// let peersBody = await this.rpcRequest("getPeerInfo", [true]);
	// let peersR = JSON.parse(peersBody);
	// peersR = peersR.result;
